...
Ny milepæl-rekke for dager-på-rad: 7, 14, 30, 50, 75, 100, deretter hver 25. dag i det uendelige (var 7/14/30/50/100, hardkodet og ulik mellom filer). Ny funksjonalitet "pausemerker": hver milepæl gir ett merke; ett merke brokobler ett helt opphold (uansett lengde, ikke per tapt dag) uten å nullstille streaken — merket forbrukes først når du faktisk logger igjen. Ingen tak på antall merker. Ren, stateless algoritme i ny `src/lib/streak.ts` — ingen ny DB-kolonne, alt utledes fra `daily_logs` hver gang. Konsoliderte 5 dupliserte streak-beregninger (log.ts, DashboardClient, StatistikkClient, TrainTodayButton, delvis StatsView via StatistikkClient) til denne ene delte funksjonen. StreakCard: milepæl-pillene viser nå et vindu (siste nådde + neste to) i stedet for hele den uendelige rekken; ny rad med skjold-ikoner for pausemerker. Toast + push + konfetti ved ny milepæl ("tjent pausemerke", med kort forklaring) og ved faktisk bruk av et merke ("pausemerke brukt"). TrainTodayButtons milepæl-toast er nå en kø (flere milepæler kan treffe samme dag). Bifangst: fikset en divide-by-zero i ProgressRing for brukere med streak ≥ 100 (nextMilestone/prevMilestone ble likt, ring-progresjon ble NaN) — løst automatisk av at milepæl-rekken nå er uendelig. `tsc --noEmit` og `npm run build` grønt. Algoritme sanity-testet mot 5 scenarioer (midt-i-pause, resume, ingen-merker-bryter, to-brokoblinger-etter-hverandre, lang rett streak) i scratchpad — alle stemte. Committet (184756b) og pushet — GitHub-integrasjonen deployet automatisk, READY på 32s, ingen feil i build-logg. Live på app.mikrotrening.no.

## 2026-09-14 | P7 | DONE — Bugfiks: pausemerker brokoblet retroaktivt (bruker Arild rapporterte 213 dager)
Bruker Arild fikk vist 213 dager på rad (skulle vært ~42-43) rett etter forrige deploy. Årsak: `calculateStreak()` hadde ingen nedre grense for hvor langt tilbake i historikken den kunne brokoble opphold — den gikk gjennom HELE `daily_logs`-historikken (tilbake til januar) og brokoblet 7 gamle opphold (fra før pausemerker eksisterte) fordi tokens tilfeldigvis var "tilgjengelige" på det tidspunktet i beregningen. Bekreftet mot ekte prod-data (bruker-id `6e301c9d-8fcc-4048-aff6-aa156dc8e948`): uten fiks streak=213/tokens=4, med fiks streak=42/tokens=3 (matcher brukerens forventning om 3 merker). Fiks: ny `PAUSE_TOKENS_LAUNCH_DATE = '2026-09-14'`-grense i `src/lib/streak.ts` — opphold kan kun brokobles av et pausemerke hvis dagen man logger igjen er på/etter lanseringsdatoen; eldre opphold brytes som før (rent kalender-sammenhengende, ingen bridging). Milepæl-/token-opptelling for den nåværende (nå korrekt beregnede) streaken er upåvirket. `tsc --noEmit` og `npm run build` grønt. Committet (78077de) og pushet, deployet automatisk — READY på 32s, ingen feil i build-logg. Live på app.mikrotrening.no.

## 2026-09-14 | P7 | DONE — Ytelse: fjernet 2 av 3 redundante auth-nettverkskall på hjemsiden
Bruker rapporterte treg oppstart (hvit skjerm → spinner → sort → hjem), spesielt ved dagens første åpning fra PWA-ikonet. Fant: `proxy.ts` (middleware), `(app)/layout.tsx` OG `(app)/page.tsx` kalte alle `supabase.auth.getUser()` uavhengig av hverandre for SAMME request — 3 sekvensielle nettverkskall til Supabase Auth (hvert med mulig token-refresh-runde) før hjemsiden i det hele tatt begynner å hente data. Verst akkurat "første åpning for dagen" siden access token da oftest må fornyes. Fiks: proxy.ts er nå eneste sted som kaller getUser() — verifisert bruker-id/e-post videreformidles til layout og page via request-header (`x-user-id`/`x-user-email`, satt med `NextResponse.next({ request: { headers } })`, bekreftet riktig mønster mot vendored Next.js-docs siden middleware heter proxy.ts i denne versjonen). layout.tsx og hjemsidens page.tsx leser nå headeren i stedet for å kalle getUser() på nytt. Cookie-forwarding (for token-refresh tilbake til nettleseren) beholdt uendret. Samme mønster kan med fordel utvides til de ~13 andre page.tsx-filene under (app)/ senere (de kaller også getUser() individuelt), men avgrenset denne runden til nøyaktig oppstarts-kjeden (proxy → layout → hjemside) som brukeren opplevde treg. `tsc --noEmit`, `npm run build` og smoke-test av uinnlogget redirect-flyt (curl mot dev-server) grønt — kunne ikke browser-verifisere innlogget flyt (ingen Playwright/Puppeteer i prosjektet). Nevnt til bruker som sekundær, ikke adressert faktor: PWA-splashen (`manifest.json background_color`) og serverless cold start på Vercel bidrar trolig også, men er ikke kodebugs å fikse på samme måte.
